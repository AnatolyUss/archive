...
class Dal
{
    ...
    /**
     * If true, print SQL queries, that led to an error.
     *
     * @var bool
     */
    protected $boolDebug;
    ...
    private function getConnectionByName($name)
    {
        if (isset($this->dbConfig[$name])) {
            $strPort = (!isset($this->dbConfig[$name]['db_port']) || empty($this->dbConfig[$name]['db_port']))
                     ? ''
                     : 'port=' . $this->dbConfig[$name]['db_port'] . ';';

            $strConnectionString = $this->dbConfig[$name]['db_driver']
                                 . ':host=' . $this->dbConfig[$name]['db_hostname'] . ';'
                                 . $strPort
                                 . 'dbname='  . $this->dbConfig[$name]['db_name']
                                 . 'charset=' . $this->dbConfig[$name]['charset'];

            $pdo = new \PDO(
                $strConnectionString,
                $this->dbConfig[$name]['db_username'],
                $this->dbConfig[$name]['db_password']
            );
    ...
    protected function bindParams($stmt)
    {
        foreach ($this->bindParamsMap as $k => &$v) {
            if (is_numeric($v)) {
                ...
                $stmt->bindParam($k, $v, \PDO::PARAM_LOB);
            } else {
